...
| `RealLocationRepository` | `:core:data` | Repository (`@Singleton`) | Obtains a real GPS fix for idle map recentering without changing the mock-position cache or cooldown. Also supplies the newest non-mock last-known fix (`lastKnownRealPosition()`) that `MapController` uses as the initial map/start position when nothing is remembered. |
...
- Each HTML page maps 1-to-1 to a feature. Update the page when the feature changes. `changelog.html` is the one page in this list that's generated rather than hand-edited — see "Regenerating the changelog" above.
