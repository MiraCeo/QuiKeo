...
- Camera follow: disabled on `REASON_API_GESTURE`. The center FAB is always available. While
  ...
  position again. While spoofing is idle or in an error state it centers at once on the newest last-known real
  fix when one exists, then requests a fresh GPS fix and moves to it unless the user panned in the
  meantime; with no last-known fix it waits for the fresh fix. If that GPS lookup
  fails with no last-known fix (permission denied, etc.), the button falls back to the last
